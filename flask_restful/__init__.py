...
def marshal(data, fields, envelope=None, display_null=True):
    ...
    for k, v in fields.items():
        tmp = marshal(data, v, display_null=display_null)\
...
class marshal_with(object):
    ...
    """
    def __init__(self, fields, envelope=None, display_null=False):
        """
    ...
    def __call__(self, f):
        ...
        def wrapper(*args, **kwargs):
            ...
            if isinstance(resp, tuple):
                data, code, headers = unpack(resp)
                return marshal(data, self.fields, self.envelope, display_null=self.display_null), code, headers
            else:
                return marshal(resp, self.fields, self.envelope, display_null=self.display_null)
        return wrapper
